...
- Параллелизм через процессы. Тяжёлое — во внешние процессы или отдельный Emacs; изоляция повышает надёжность.
...
Практика: держите обработчики максимально короткими; тяжёлое выполняйте после завершения процесса или в отдельном процессе/async.el; обновления UI планируйте через run-at-time 0.
...
- Для тяжёлого — async.el (отдельный Emacs) или внешние утилиты.
...
Этот шаблон устраняет накопление параллельных запусков. Для тяжелого кода async-start даст реальный параллелизм (отдельный Emacs‑процесс).
...
Потоки (make-thread) в Emacs — кооперативные: за интерпретатор держится глобальная блокировка. Они не ускоряют тяжёлые задачи, но полезны, когда нужно «ждать что-то» без блокировки главного потока, координировать таймауты, организовать ожидание условных переменных. Любая работа с буферами/окнами/кадрами должна выполняться в главном потоке. Используйте run-at-time 0 для безопасного UI‑обновления.
...
- I/O — через процессы/сокеты. Тяжёлое — во внешний процесс или отдельный Emacs. Не пытайтесь «ускорить» лисп‑потоками.
...
- Тяжёлые CPU‑задачи — внешний процесс или отдельный Emacs (c.el).
